 ...
 */
abstract class PluginBandConfigForm extends BaseForm
{
  protected
  ...
  public function setup()
  {
    ...
    $this->widgetSchema->setNameFormat('band_config[%s]');
...
/*      $this->setWidget($name, new sfWidgetFormChoice(array('choices' => $choices, 'expanded' => true)));
      ...
      $this->widgetSchema->setHelp($name, $i18n->__('Send a notice mail to administrator when new member joined the %band%.'));

      $default = $this->band->getConfig($name);
...
*/      $this->setDefault($name, 0);
  ...
  public function setConfigWidget($name)
  {
    ...
    $bandConfig = Doctrine::getTable('BandConfig')->retrieveByNameAndBandId($name, $this->band->getId());
    if ($bandConfig)
    {
      $this->setDefault($name, $bandConfig->getValue());
    }
  ...
  {
    $categories = sfConfig::get('openpne_band_category');
  ...
  public function save()
  {
    ...
    {
      $config = Doctrine::getTable('BandConfig')->retrieveByNameAndBandId($key, $this->band->getId());
      if (!$config)
      {
        $config = new BandConfig();
        $config->setBand($this->band);
        $config->setName($key);
      }
