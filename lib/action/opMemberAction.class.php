...
abstract class opMemberAction extends sfActions
{
  public function isSecure(){
    ...
    if (!( ($member = $this->getUser()->getMember()) && $member->getIsActive() )
        && ('homepage' === $current_routing) )
    {
      $this->redirect('@event_top');
    }
    ...
    // Need login
    if (('member_config_image' === $current_routing) ||
        ('member_delete' === $current_routing) ||
        ('login' === $current_routing) ||
        ('member_logout' === $current_routing) ||
        ('member_delete' === $current_routing) ||
        ('member_config' === $current_routing) ||
        ('member_config_image' === $current_routing) ||
        ('member_config_jsonapi' === $current_routing)){
      return true;
    }else{
      return false;
    }
  ...
  public function executeLogin($request)
  {
    ...
    if ($request->hasParameter('authMode'))
    {
      ...
      // edit check after first login
     $updated = false;
     strtotime($this->getUser()->getMember()->getCreatedAt()) .'sec<br>';
      // time diff check 0 or 1 sec
      if(strtotime($this->getUser()->getMember()->getUpdatedAt()) -
         strtotime($this->getUser()->getMember()->getCreatedAt()) > 1)
      {
        $updated = true;
      }
      $member_profiles = Doctrine::getTable('MemberProfile')->getProfileListByMemberId($this->getUser()->getMemberId());;
      foreach ($member_profiles as $profile)
      {
        $time_diff = strtotime($profile->getUpdatedAt()) -
                     strtotime($profile->getCreatedAt());
        if($time_diff > 1)
        {
          $updated = true;
        }
